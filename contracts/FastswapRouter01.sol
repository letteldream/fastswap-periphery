...
contract FastswapRouter01 is IFastswapRouter01 {
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'FastswapRouter01: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = FastswapLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'FastswapRouter01: INSUFFICIENT_A_AMOUNT');
    ...
    function removeLiquidity(
        ...
        require(amountA >= amountAMin, 'FastswapRouter01: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'FastswapRouter01: INSUFFICIENT_B_AMOUNT');
    }
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'FastswapRouter01: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, FastswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        _swap(amounts, path, to);
    }
    ...
    {
        require(path[0] == WETH, 'FastswapRouter01: INVALID_PATH');
        amounts = FastswapLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'FastswapRouter01: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'FastswapRouter01: INVALID_PATH');
        amounts = FastswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'FastswapRouter01: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, FastswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'FastswapRouter01: INVALID_PATH');
        amounts = FastswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'FastswapRouter01: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(FastswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public pure override returns (uint amountIn) {
        return FastswapLibrary.getAmountOut(amountOut, reserveIn, reserveOut);
    }
